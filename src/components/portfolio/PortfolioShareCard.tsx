...
export default function PortfolioShareCard() {
  ...
  <svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
    ...
    <text x="110" y="245" fill="white" font-family="Inter, Arial" font-size="64" font-weight="900">${username}</text>
    <text x="112" y="292" fill="#9fb2c4" font-family="Inter, Arial" font-size="28">${wallet}</text>
    ...
    <text x="860" y="438" fill="#ff4fc8" font-family="Inter, Arial" font-size="52" font-weight="900">${stats.top?.token ?? "N/A"}</text>
  </svg>`;

  const shareToX = () => {
    const text = encodeURIComponent(`My Lunexis portfolio is live: ${username} | Top asset ${stats.top?.token ?? "N/A"} | XP ${profile?.xp ?? 0}`);
  ...
  return (
    <section className="lunexis-premium-card">
      ...
      <div className="flex flex-wrap gap-3 mt-4">
        <button onClick={() => downloadSvg("lunexis-portfolio-card.svg", svg)}>Download Card</button>
